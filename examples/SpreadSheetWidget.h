...
template <typename T> class SpreadSheet : public Fl_Table {
public:
...
template <typename T>
SpreadSheet<T>::SpreadSheet(int X, int Y, int W, int H, const char *L)
  : Fl_Table(X, Y, W, H, L) {
...
// Apply value from input widget to values[row][col] array and hide (done editing)
template <typename T> void SpreadSheet<T>::set_value_hide() {
...
//    and make the widget 'appear' at the cell's location.
//
template <typename T> void SpreadSheet<T>::start_editing(int R, int C) {
...
// Tell the input widget it's done editing, and to 'hide'
template <typename T> void SpreadSheet<T>::done_editing() {
...
template <typename T> void SpreadSheet<T>::set_data(vector<T> *data) {
  m_Data = data;

  update_size();
}

template <typename T> int SpreadSheet<T>::handle(int event) {
  int R = callback_row();
  int C = callback_col();
...
template <typename T> string SpreadSheet<T>::col_header_txt(int index) {
...
template <typename T> void SpreadSheet<T>::event_callback2() {
  int R = callback_row();
  int C = callback_col();
...
template <typename T> int SpreadSheet<T>::copy(int clipboard) {
  int rstart, cstart;
  int rend, cend;
...
template <typename T> void SpreadSheet<T>::paste(const char *str, int len) {
  int datarow = get_data_nrow();
  int datacol = get_data_ncol();

  vector < vector < string > > table;
  parse_table(str, len, table);
...
template <typename T> void SpreadSheet<T>::update_size() {
  int r = rows();
  int c = cols();
  rows(get_data_nrow());
  cols(get_data_ncol());
...
template <typename T> string SpreadSheet<T>::get_value(int R, int C) {
  return to_string((*m_Data)[R]);
}

template <typename T> string SpreadSheet<T>::get_exact_value(int R, int C) {
  return get_value(R, C);
}

template <typename T> int SpreadSheet<T>::get_data_ncol() {
  if (get_data_nrow() > 0) {
    return 1;
  }
  return 0;
...
template <typename T> void SpreadSheet<T>::update_size(int nrow, int ncol) {
  bool change = false;
...
typedef SpreadSheet<vector<string> > SpreadSheetString;
